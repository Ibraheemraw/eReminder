...
class FavoriteCell: UICollectionViewCell, Expandable {
    ...
    func hide(in collectionView: UICollectionView, frameOfSelectedCell: CGRect) {
        ...
        let newYValue: CGFloat
        if currentY < frameOfSelectedCell.origin.y {
            let offset = frameOfSelectedCell.origin.y - currentY
            newYValue = collectionView.contentOffset.y - offset
        } else {
            let offset = currentY - frameOfSelectedCell.maxY
            newYValue = collectionView.contentOffset.y + collectionView.frame.height + offset
        }
        self.frame.origin.y = newYValue
        layoutIfNeeded()
    }
    func show() {
